...
export const api = {
  ...
  skills: {
    saved: () => apiClient.get('/skills/saved'),
    save: (id: string) => apiClient.post(`/skills/${encodeURIComponent(id)}/save`),
    unsave: (id: string) => apiClient.delete(`/skills/${encodeURIComponent(id)}/save`),
    list: (params?: Record<string, unknown>) => apiClient.get('/skills', { params }),
    categories: () => apiClient.get('/skills/categories'),
    create: (payload: { title: string; description?: string; categoryId: string; isActive?: boolean }) => apiClient.post('/skills', payload),
    update: (id: string, payload: { title?: string; description?: string; categoryId?: string; isActive?: boolean }) => apiClient.put(`/skills/${encodeURIComponent(id)}`, payload),
    remove: (id: string) => apiClient.delete(`/skills/${encodeURIComponent(id)}`),
  },
  users: {
    profile: () => apiClient.get('/profile'),
    publicProfile: (id: string) => apiClient.get(`/users/${encodeURIComponent(id)}`),
    publicInterests: (id: string) => apiClient.get(`/users/${encodeURIComponent(id)}/interests`),
    blockStatus: (id: string) => apiClient.get(`/users/${encodeURIComponent(id)}/block-status`),
    block: (id: string) => apiClient.post(`/users/${encodeURIComponent(id)}/block`),
    unblock: (id: string) => apiClient.delete(`/users/${encodeURIComponent(id)}/block`),
    ...
    addInterest: (skillId: string) => apiClient.post('/profile/interests', { skillId }),
    removeInterest: (skillId: string) => apiClient.delete(`/profile/interests/${encodeURIComponent(skillId)}`),
  },
  ...
  requests: {
    ...
    review: (id: string, payload: { rating: number; comment?: string }) => apiClient.post(`/requests/${encodeURIComponent(id)}/review`, payload),
  },
  conversations: {
  ...
  messages: {
    ...
    create: (conversationId: string, content: string) => apiClient.post(`/conversations/${encodeURIComponent(conversationId)}/messages`, { content }),
  ...
  moderation: {
    createReport: (payload: { targetUserId: string; reason: string }) => apiClient.post('/moderation/reports', payload),
  },
  admin: {
    ...
    updateSkillStatus: (id: string, isActive: boolean) => apiClient.put(`/admin/skills/${encodeURIComponent(id)}/status`, { isActive }),
    createCategory: (payload: { name: string; slug: string; description?: string }) => apiClient.post('/admin/categories', payload),
    updateCategory: (id: string, payload: { name?: string; slug?: string; description?: string }) => apiClient.put(`/admin/categories/${encodeURIComponent(id)}`, payload),
    deleteCategory: (id: string) => apiClient.delete(`/admin/categories/${encodeURIComponent(id)}`),
