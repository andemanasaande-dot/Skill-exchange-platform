...
export function ConfirmDialog({ open, title = 'Confirm action', message, error, pending = false, onConfirm, onClose }: { open: boolean; title?: string; message: string; error?: string; pending?: boolean; onConfirm: () => void; onClose: () => void }) { return <Modal open={open} title={title} onClose={pending ? () => undefined : onClose}><p className="text-sm text-slate-600">{message}</p>{error && <div className="mt-4"><ErrorState message={error} /></div>}<div className="mt-6 flex justify-end gap-3"><button type="button" className="button-secondary" onClick={onClose} disabled={pending}>Cancel</button><button type="button" className="button-primary" onClick={onConfirm} disabled={pending}>{pending ? <LoaderCircle className="animate-spin" size={16} /> : <Check size={16} />}{pending ? 'Confirming...' : 'Confirm'}</button></div></Modal>; }
