...
export const moderationRepository = {
  ...
  createReport: async (payload: { reporterId: string; targetUserId?: string; reason: string; entityType?: 'USER' | 'SKILL' | 'MESSAGE'; entityId?: string }) => prisma.$transaction(async (tx: Prisma.TransactionClient) => {
  ...
  reviewReport: async (id: string, status: 'UNDER_REVIEW' | 'RESOLVED' | 'DISMISSED', actorUserId: string, actorRole: 'MODERATOR' | 'ADMIN', resolution?: string) => prisma.$transaction(async (tx: Prisma.TransactionClient) => {
  ...
  applyUserAction: async (userId: string, action: 'WARN' | 'RESTRICT' | 'SUSPEND' | 'BAN', actorUserId: string, reason?: string, durationHours?: number) => prisma.$transaction(async (tx: Prisma.TransactionClient) => {
